...
public abstract class AbstractJavassistTool implements ClassFileTransformer {
    ...
    public static List<BasicBlock> getBasicBlocks(CtBehavior behavior) throws BadBytecode {
        ...
        for (Block block : new ControlFlow(behavior.getDeclaringClass(), behavior.getMethodInfo()).basicBlocks()) {
            ...
            bbs.add(new BasicBlock(behavior, block.position(), ainfo.toLineNumber(block.position()), block.length(),
                    entrances));
        }
    ...
    @Override
    final public byte[] transform(ClassLoader loader, String className, Class<?> classBeingRedefined,
